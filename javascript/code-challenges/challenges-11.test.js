...
const count = (target, input) => {
  return input
    ...
    map(subArray => subArray.filter(num => num === target))
    .
...
const divisibleByFiveTwoToThePower = (input) => {
  return input
    .map(subArray => subArray.filter(num => typeof num === 'number' && num % 5 === 0))
    .map(filteredSubArray => filteredSubArray.map(num => Math.pow(2, num)));
};
...
let findMaleAndFemale = (data) => {
  ...
  return maleAndFemaleCharacters.map(character => character.name).join(' and ');
};
